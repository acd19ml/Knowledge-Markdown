...
> "学习策略指智能体学习如何管理记忆的方式：存储什么、何时存储、如何表示、何时检索或丢弃，以及存储在何处或从何处检索 —— 而非依赖固定的手工启发式方法。此类策略通常从数据或反馈中优化（如监督信号、强化学习或自我改进）。"
...
| 范式 | 学习信号 | 核心优势 | 核心局限 |
|---|---|---|---|
| **提示工程** | 自然语言指令（无梯度更新） | 无需昂贵微调；高度可解释 | 无显式信用分配；长期优化能力有限 |
| **微调（SFT）** | 监督标签/精心整理的数据 | 稳定、可复用的记忆行为嵌入权重 | 训练后固定；无法适应新奖励 |
| **强化学习** | 任务结果奖励/交互反馈 | 显式信用分配；下游任务结果塑造记忆决策 | 样本效率；奖励设计复杂度 |
...
**局限性**：语言中介；缺乏显式信用分配 → 相比微调/RL，长期策略优化能力有限。
...
> "在这个范围内，强化学习是必不可少的，因为只有长期和跨情节的奖励信号才能决定哪些记忆应该持久、适配或修订。"
