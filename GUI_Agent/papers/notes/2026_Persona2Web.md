...
- **Links**: [PDF](../source/Persona2Web.pdf) | Code: placeholder mentioned in PDF | Project: not listed in PDF
...
- **My observed limitations**: 
  Persona2Web 更像“读用户历史”的 benchmark，而不是“维护用户历史”的方法论文。它证明 personalized web agents 在 ambiguity resolution 上仍极弱，但用户历史是合成的、task success 也整体偏低，因此目前更适合作为 A-3 / user-memory 的评测入口，而不是主线方法证据。
...
- **Corresponding survey section/category**:
  这篇最适合放在 **Personalization / User Modeling / Evaluation** 小节，作为 “user history as memory” 方向的 benchmark 入口。按当前 main-line，它属于外围支线，不会改变 A-1 + A-4 的核心叙事。
...
- Gap signal 3: reasoning-aware evaluation 明显优于 action-wise / outcome-based（Table 4, p.6）→ personalization 评估若不看 reasoning，会严重低估失败原因复杂度。
  ...
  如果后续要展开 A-3，这篇会很重要，因为它把“读取已有历史”和“真正会用历史”区分开了。但它仍停留在 access / readout 层，没有进入持续维护和更新 personalization memory 的问题。

### Reusable Elements
- **Methodology**: clarify-to-personalize query design、reasoning-aware personalization rubrics、history access scheme 对照。
  最值得复用的是评估设计，尤其是把“个性化失败”和“导航失败”拆开。对知识库写作来说，这能避免把所有低成功率都误归因成执行问题。
- **Experimental design**: 同时报 `P_web`、`P_pref`、intent satisfaction、SR，是个性化 agent 评测里很完整的一组指标。

### Connections to Other Papers in Knowledge Base
  它与 [2026_AmbiBench.md](/Users/mac/studyspace/Knowledge-Markdown/GUI_Agent/papers/notes/2026_AmbiBench.md) 可以一起构成 ambiguity / clarification 的移动端与 web 端对照；与 [2026_M2.md](/Users/mac/studyspace/Knowledge-Markdown/GUI_Agent/papers/notes/2026_M2.md) 对照时，则能看出 history / memory 在 personalization 和 long-horizon navigation 中承担的是两类完全不同的功能。
