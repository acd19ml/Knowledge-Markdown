...
> [PDF](./ExpeL.pdf) | [源码](https://github.com/LeapLabTHU/ExpeL) | [提取文本](./ExpeL_clean.txt)

## Meta
- **Title**: ExpeL: LLM Agents Are Experiential Learners
- **Authors**: Andrew Zhao et al. | Tsinghua University (BNRist)
- **Venue**: AAAI 2024 | arXiv:2308.10144v3 [cs.LG]
- **Links**: [PDF](../source/ExpeL.pdf) | [Code](https://github.com/LeapLabTHU/ExpeL) | [Project](https://andrewzh112.github.io/expel)
...
> ExpeL 的优点是可解释、无需参数更新、并且真正把跨任务 insight 写成了可检索对象；但按当前主线看，它仍属于 bridge work。问题不在于它有没有 memory，而在于 insight 的写入、检索和更新仍较启发式，且文本环境证据不足以直接外推到 GUI procedural memory。
...
> 这篇应放在 `Self_Evolve` 的 “Experience-to-Policy without Weight Update” 小节，作为 Reflexion 和更结构化 skill/memory 方法之间的桥接论文。它对当前 main-line 的价值，在于说明 inference-time external memory 是可行的，但尚未形成 failure-aware procedural rule system。
...
> 对当前主线而言，最值得提炼的不是多个分散 gap，而是一个合并问题：如何让跨任务 memory 的写入、检索和更新变得更鲁棒、更可解释，且不依赖纯启发式 insight pooling。

### Reusable Elements
- **Methodology**:
  - 三阶段闭环（gather -> extract -> infer）可直接迁移到你的 agent self-evolution pipeline。
  - insight 维护的“可编辑/可投票”机制适合做人类可干预 memory governance。
> 更贴近当前主线的做法，是把 ExpeL 的 `insight` 继续升级成可执行 skill schema，补上触发条件、动作模板、失败回滚和修订规则，这样才更接近 GUI procedural memory。
- **Experimental design**:
  - 强烈建议复用其 “single-attempt vs reattempt” 分离评估：可测真正的跨任务学习能力，而不是在线 retry 带来的即时补救收益。

### Connections to Other Papers in Knowledge Base
> 它与 [2026_SkillRL.md](/Users/mac/studyspace/Knowledge-Markdown/Self_Evolve/papers/notes/2026_SkillRL.md) 形成 prompt-memory vs RL skill evolution 的对照，与 [2023_Reflexion.md](/Users/mac/studyspace/Knowledge-Markdown/Self_Evolve/papers/notes/2023_Reflexion.md) 则是单任务自改进向跨任务积累的延伸。放到 GUI 侧时，它可以与 [2024_MobileAgentV2.md](/Users/mac/studyspace/Knowledge-Markdown/GUI_Agent/papers/notes/2024_MobileAgentV2.md) 和 [2026_MobileAgentV3_5.md](/Users/mac/studyspace/Knowledge-Markdown/GUI_Agent/papers/notes/2026_MobileAgentV3_5.md) 对照，作为经验机制设计的前导蓝图。
...
> CS6493 课程项目 [proposal](../CS6493-proposal.md) 包含 Reflexion + ExpeL 的对比复现规格，本节为 ExpeL 独立深化框架。

### 需要验证的核心声明

| # | 声明 | 来源 | 验证方式 |
|---|------|------|---------|
| C1 | task similarity 检索优于 reasoning similarity / random | Table 3 lower p.10 | 消融三种检索策略，固定其余变量 |
| C2 | insight 质量：gpt-4 优于 gpt-3.5，reflections 并入 insight 反而掉点 | Table 3 upper p.10 | 对比不同 insight 生成器 |
| C3 | ExpeL 单次尝试（R0）显著优于 ReAct+Reflexion | Table 2 p.9 | 在 ALFWorld 上分离 R0 评估 |
| C4 | 经验多样性关键：只靠 few-shot 抽 insight 基本无效 | Fig.6 p.9 | 对比不同经验池规模（10/50/100 task） |

### Phase 1：读源码，理解架构

- [ ] clone 仓库，梳理三阶段：experience gathering → insight extraction → task inference
- [ ] 理解 FAISS + all-mpnet-base-v2 的检索实现（embedding 维度、index 类型）
- [ ] 理解 ADD/EDIT/UPVOTE/DOWNVOTE 的 insight 维护逻辑和 importance 计数机制

### Phase 2：ALFWorld 复现

- [ ] 用 training split（50-100 task）收集经验，抽取 insight
- [ ] 用 evaluation split 跑 task inference，对比 ReAct 和 Reflexion baseline
- [ ] 分离 R0 和 R3 评估，观察跨任务学习 vs 在线 retry 的各自贡献

### Phase 3：消融实验

- [ ] **检索策略消融（C1）**：task-sim vs reasoning-sim vs random，固定 insight 模块
- [ ] **经验池规模曲线（C4）**：10/50/100 task 经验，观察 insight 质量和 SR 变化
- [ ] **FEVER transfer**：用 HotpotQA insight 迁移到 FEVER，验证跨域 transfer 能力

### Phase 4：Critical Reflection

- [ ] insight 的 ADD/EDIT 机制是否真的在迭代改进，还是只是堆积？如何衡量 insight 质量？
- [ ] 和 Reflexion 的本质区别：单任务内反思 vs 跨任务 insight 积累 — 用实验数据说话
- [ ] ExpeL 的 insight 离 AWM 的 workflow 有多远？差在哪里（结构化程度？可执行性？）

## 实验记录

```
experiments/
├── 01-env-setup.md                  环境搭建 + FAISS index 验证
├── 02-experience-gathering.md       经验收集阶段（ALFWorld training split）
├── 03-insight-extraction.md         insight 质量分析
├── 04-task-inference.md             ALFWorld 评估，R0/R3 分离
├── 05-retrieval-ablation.md         检索策略消融
└── 06-fever-transfer.md             跨域迁移实验
```
