...
> [PDF](./Reflexion.pdf) | [源码](https://github.com/noahshinn024/reflexion) | [提取文本](./Reflexion_clean.txt)
...
> CS6493 课程项目 [proposal](../CS6493-proposal.md) 包含此论文的复现规格，本节为独立深化框架。

### 需要验证的核心声明

| # | 声明 | 来源 | 验证方式 |
|---|------|------|---------|
| C1 | self-reflection 相比仅加 episodic memory 多带来 +8% | Fig.4(c) p.7 | 消融：retry-only vs retry+memory vs retry+memory+reflection |
| C2 | 测试生成 + 语言反思缺一不可（Rust hardest-50） | Table 3 p.8 | 分别去掉 test-gen / reflection 观察 pass@1 变化 |
| C3 | 弱模型（starchat-beta）上 Reflexion 无增益 | Appendix A p.12 | 跑小模型（Llama-3 8B）对比 GPT-4o-mini |
| C4 | WebShop 4 轮后仍无显著提升 | Appendix B.1 p.14 | 跑 WebShop，记录每轮 SR 曲线 |

### Phase 1：读源码，理解架构

- [ ] clone 仓库，理解 Actor / Evaluator / Self-Reflection 三模块拆分
- [ ] 梳理 ALFWorld 和 HotpotQA 的 evaluator 实现（启发式 vs LLM 判别的区别）
- [ ] 理解 sliding window memory 的实现细节（`Ω` 参数如何控制）

### Phase 2：ALFWorld 复现

- [ ] 用 50 task 子集，跑 ReAct baseline 和 Reflexion，对比 SR（R1/R2/R3）
- [ ] 记录每轮的 reflection 文本质量（是否真的在定位问题）
- [ ] 记录：论文 SR vs 实际 SR，分析 gap（环境版本/模型版本差异）

### Phase 3：验证边界条件

- [ ] **弱模型实验（C3）**：用 Groq 免费 API 跑 Llama-3.3-70B，观察 Reflexion 增益是否消失
- [ ] **WebShop 天花板（C4）**：跑 4 轮，画 SR 曲线，分析为什么反思无法帮助跳出局部最优
- [ ] **Evaluator 质量实验**：人为注入错误的 success 判断，观察 reflection 方向是否被误导

### Phase 4：Critical Reflection

- [ ] Reflexion 的反思文本有多大比例真正定位了错误根因？有多少是泛泛描述？
- [ ] `trial -> verbalize -> retry` 和 AWM 的 `trajectory -> induce workflow -> reuse` 在机制上有什么本质区别？
- [ ] 如果把 reflection 结构化（错误类型 + 证据状态 + 修复动作），能否减少 local minima？

## 实验记录

```
experiments/
├── 01-env-setup.md              ALFWorld 环境搭建 + 源码结构
├── 02-alfworld-baseline.md      ReAct baseline 复现
├── 03-alfworld-reflexion.md     Reflexion 复现，SR 曲线
├── 04-weak-model.md             Llama-3 弱模型实验
└── 05-webshop-ceiling.md        WebShop 天花板分析
```
