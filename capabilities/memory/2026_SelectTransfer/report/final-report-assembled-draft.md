...
> **Supporting materials.** Every quantitative claim in this report can be traced through three layers:
...
LLM-based agents increasingly use past experience as external memory to improve future task performance. However, standard evaluations report average benchmark gains, which do not distinguish whether memory helps because it is relevant or simply because more context is present. This project studies **selective transfer**: whether memory helps on structurally matched tasks while avoiding harm on mismatched tasks. We operationalize this in a near-transfer setting (HotpotQA → 2WikiMultiHopQA) with a local 9B-parameter model, comparing three conditions — No Memory, Episodic Trace, and Cross-Episode Consolidation — across predefined Relevant and Irrelevant splits.

Round 1, the pilot phase, did not produce a headline benchmark gain. Instead, it revealed that early evidence of negative transfer was an artifact of the experimental infrastructure rather than a genuine memory failure. Through a controlled chain of single-variable repairs — prompt scaffolding, role-aware case discipline, subtype-aware pairing, and executable operator abstraction — the project recovered the relevant memory path on its strongest diagnostic case for both memory forms. The main contribution is methodological: selective transfer claims are only as reliable as the operational definition of relevance and the executability of the memory abstraction. Without fine-grained pairing, case-role discipline, and executable memory representations, the experiment risks measuring its own setup artifacts rather than the phenomenon of interest.
...
A central question in memory-augmented AI is not simply whether past experience can be stored, but when stored experience becomes **reusable knowledge** — something that selectively improves future performance rather than acting as inert context or, worse, a source of interference.
...
This framing requires a controlled split. For every target task, we predefine:

- a **Relevant** pairing — source memory and target share the same reasoning subtype
- an **Irrelevant** pairing — source memory and target belong to deliberately different reasoning subtypes
...
The long-term question — when does experience transform into reusable knowledge, and further into a lasting influence on an agent's judgment — is too broad for a single course project. This project therefore cuts a narrower empirical slice: a near-transfer QA setting with a small model, a small number of controlled conditions, and a fixed experience budget. The goal is not to resolve the long-term question but to build a measurement workflow that can distinguish genuine selective transfer from confounds — and to identify which infrastructure components must be in place before any efficacy claim can be made.

Round 1, the pilot phase reported here, focuses entirely on this measurement-building goal. It does not produce a large-scale empirical result. It produces a methodological demonstration: that apparent negative transfer can be an artifact of coarse pairing granularity and non-executable memory abstractions, and that careful single-variable repair can overturn an incorrect conclusion without changing the model itself.

### 1.4 Project Contribution

The project's contribution is threefold:

1. **An evaluation perspective.** Not average gain, but split-specific selective transfer — Relevant and Irrelevant reported separately, never averaged.
2. **A reusable protocol.** Taxonomy, source-set construction, frozen pairing, role-aware case discipline, and iterative single-variable diagnosis — a pipeline that makes each experimental decision traceable.
3. **A route-selection signal.** Whether future work should focus on memory *form* (how experience is represented), memory *use* (how experience is gated), or their coupling.
...
### 2.3 Source Set Construction and Pairing Protocol

From HotpotQA, we constructed source memory sets of N = 5 tasks each, all from the same reasoning cluster. Sets satisfy: entity-disjoint within set, low lexical overlap, no answer leakage into target tasks.
...
### 2.7 Evaluation Protocol Design

Three protocol-level design decisions emerged during Round 1 and are central to the method:

**Case-role discipline.** The initial smoke subset showed that a flat average over all target cases was misleading. Different cases served different methodological roles — process sanity, diagnostic, or audit/boundary. After Round 1c, mixed-role aggregation was explicitly banned. Every case must be classified into its experimental role before entering any summary table.

**Subtype-aware pairing.** The original `bridge` cluster was found to be too coarse — it conflated `attribute_bridge` and `relation_chain_bridge`. A coarse-grained "relevant" label could still be structurally mismatched at the subtype level. Round 1d–1g redefined relevance at the subtype level.

**Executable abstraction requirement.** For Cross-Episode Consolidation, topical relevance alone was insufficient. The memory artifact also had to preserve the target task's **operator structure** in an executable form. A natural-language summary of the relation pattern was not enough; the artifact had to encode candidate operator branches explicitly enough for the model to execute them.
...
### 3.1 Making the Experiment Observable
...
*Evidence: [L2 §C1] → result CSV, raw outputs for wiki_dev_8896.*

### 3.2 Role-Aware Case Analysis

Round 1c reclassified the 6 smoke cases by experimental role:
...
*Evidence: [L2 §C2] → role classification CSV, aggregate overview CSV.*

### 3.3 Pairing Granularity Created a False Negative
...
### 3.4 Consolidation Required Operator-Level Repair

Subtype-aware rerouting did not automatically repair consolidation. The failure mode shifted: the model now referenced the memory but could not resolve the kinship operator `sibling-in-law`.

Two further sub-rounds isolated the cause:

- **Round 1h** fixed formatting and branch-wording issues in the consolidation artifact. Output structure stabilized, but the answer remained wrong. The failure was narrowed to **kinship-operator interpretation**: the heuristic left `sibling-in-law` as a vague natural-language label.
...
### 3.5 The Full Repair Chain

`wiki_dev_2639` is the single most informative case in Round 1. It simultaneously satisfies all conditions for a full before/after interpretation shift: the no-memory baseline is correct, relevant memory initially degrades, subtype-aware rerouting repairs episodic memory, operator-aware repair repairs consolidation, and irrelevant memory remains wrong throughout.
...
1. **Repaired evaluation at moderate scale.** Apply the corrected pairing and operator-aware artifacts to a larger target set (20–40 cases) to test whether the patterns observed on `wiki_dev_2639` generalize across relation-chain bridge tasks. The next round should ask a much cleaner question than reopening average-gain comparisons: *does the repaired workflow transfer beyond one key diagnostic case?*
