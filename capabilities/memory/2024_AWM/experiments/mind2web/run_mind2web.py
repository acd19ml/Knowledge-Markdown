...
def main():
    ...
    if args.start_idx >= len(examples):
        raise ValueError(
            f"start_idx={args.start_idx} is out of range for website [{args.website}] "
            f"with only {len(examples)} examples."
        )

    if args.end_idx is None or args.end_idx > len(examples):
