...
from utils.openai_client import DEFAULT_MAX_TOKENS
...
def eval_sample(task_id, args, sample):
    ...
    for s, act_repr in zip(sample["actions"], sample["action_reprs"]):
        ...
        model_max_tokens = MAX_TOKENS.get(args.model, DEFAULT_MAX_TOKENS)
        if total_num_tokens > model_max_tokens:
            logger.info(
                f"Too many tokens in acting ({total_num_tokens} / {model_max_tokens}), skipping..."
            )
            element_acc.append(0)
            action_f1.append(0)
            step_success.append(0)
