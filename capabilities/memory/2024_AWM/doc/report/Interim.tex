...
\title{Interim Report: Experience-Dependent Procedural Knowledge in GUI Agents: A Taxonomy and Research Agenda}
\author{Li Mengxiao}
\date{\today}
...
\subsection{Why This Problem Matters}\label{21-why-this-problem-matters}
...
AppAgent~\parencite{zhang2023appagentmultimodalagentssmartphone},
MobileGPT~\parencite{lee2024exploreselectderiverecall},
MAGNET~\parencite{sun2026magnetadaptiveguiagents},
ActionEngine~\parencite{zhong2026actionenginereactiveprogrammaticgui},
IntentCUA~\parencite{lee2026intentcualearningintentlevelrepresentations}, and GUI-Owl-1.5 /
MobileAgentV3.5~\parencite{xu2026mobileagentv35multiplatformfundamentalgui} have shown that
...
\subsection{Contributions of This Survey}\label{24-contributions-of-this-survey}
...
\section{Background and Problem Formulation}\label{3-background-and-problem-formulation}
...
abstractions, as illustrated by AppAgent~\parencite{zhang2023appagentmultimodalagentssmartphone},
MobileGPT~\parencite{lee2024exploreselectderiverecall},
MobileAgent-v2~\parencite{wang2024mobileagentv2mobiledeviceoperation},
MAGNET~\parencite{sun2026magnetadaptiveguiagents},
ActionEngine~\parencite{zhong2026actionenginereactiveprogrammaticgui}, and
IntentCUA~\parencite{lee2026intentcualearningintentlevelrepresentations}. This body of work therefore
...
Agents~\parencite{park2023generativeagentsinteractivesimulacra},
MemGPT~\parencite{packer2024memgptllmsoperatingsystems}, and
A-MEM~\parencite{xu2025amemagenticmemoryllm}. Without this
...
Agents~\parencite{park2023generativeagentsinteractivesimulacra},
MemGPT~\parencite{packer2024memgptllmsoperatingsystems}, and
A-MEM~\parencite{xu2025amemagenticmemoryllm} show that memory can be externally stored,
...
\subsubsection{Self-Evolving Agents}\label{313-self-evolving-agents}
...
storage to transformation. AWM~\parencite{wang2024agentworkflowmemory} shows that
trajectories can be distilled into reusable workflows instead of being
replayed verbatim. ExpeL~\parencite{zhao2024expelllmagentsexperiential} shows how
success-failure contrasts can maintain an editable pool of insights.
SkillRL~\parencite{xia2026skillrlevolvingagentsrecursive} shows that failures can be distilled
into generalized skills rather than left as raw cautionary traces.
AgentKB~\parencite{tang2025agentkbleveragingcrossdomain} shows that experience can transfer
...
\subsection{Common Confusions to Eliminate}\label{33-common-confusions-to-eliminate}
...
\subsection{Dimension I: What Is Learned}\label{41-dimension-i-what-is-learned}
...
\subsection{Dimension II: When Is Experience Written Back}\label{42-dimension-ii-when-is-experience-written-back}
...
\subsection{Dimension III: How Is Experience Reused}\label{43-dimension-iii-how-is-experience-reused}
...
\subsection{Interpretive Validity of the Taxonomy}\label{44-interpretive-validity-of-the-taxonomy}
...
\subsection{Coverage Pattern of Current Systems}\label{45-coverage-pattern-of-current-systems}
...
~\parencite{sun2026magnetadaptiveguiagents}, which shows that GUI agents can indeed summarize
...
\section{Related Work Through the Taxonomy}\label{5-related-work-through-the-taxonomy}
...
\subsection{Pre-Task Exploration and App-Bound Knowledge}\label{51-pre-task-exploration-and-app-bound-knowledge}

Early GUI memory systems primarily use memory to support environment
familiarization. AppAgent~\parencite{zhang2023appagentmultimodalagentssmartphone} is the clearest
...
MobileGPT~\parencite{lee2024exploreselectderiverecall} pushes this line further by
...
\subsection{In-Task Reflection, Compression, and Local Repair}\label{52-in-task-reflection-compression-and-local-repair}
...
MobileAgent-v2~\parencite{wang2024mobileagentv2mobiledeviceoperation} is representative
...
MobileAgentV3.5~\parencite{xu2026mobileagentv35multiplatformfundamentalgui} and
M2~\parencite{yan2026m2dualmemoryaugmentationlonghorizon}. Both show that context compression and
...
\subsection{Post-Task Workflow Memory as the Current Best GUI-Side Partial Solution}\label{53-post-task-workflow-memory-as-the-current-best-gui-side-partial-solution}
...
intent-level memory. MAGNET~\parencite{sun2026magnetadaptiveguiagents} is currently the
...
ActionEngine~\parencite{zhong2026actionenginereactiveprogrammaticgui} and
IntentCUA~\parencite{lee2026intentcualearningintentlevelrepresentations} extend this shift in two
...
\subsection{Transferable Precedents from Memory and Self-Evolving Agents}\label{54-transferable-precedents-from-memory-and-self-evolving-agents}
...
Agents~\parencite{park2023generativeagentsinteractivesimulacra} and
MemGPT~\parencite{packer2024memgptllmsoperatingsystems} establish two complementary
...
A-MEM~\parencite{xu2025amemagenticmemoryllm} extends this argument by showing that
...
On the self-evolving side, AWM~\parencite{wang2024agentworkflowmemory},
ExpeL~\parencite{zhao2024expelllmagentsexperiential},
SkillRL~\parencite{xia2026skillrlevolvingagentsrecursive}, and
AgentKB~\parencite{tang2025agentkbleveragingcrossdomain} provide even stronger precedents
...
\subsection{User-Centric Personalization as a Peripheral but Confirmed Line}\label{55-user-centric-personalization-as-a-peripheral-but-confirmed-line}

User-centric memory remains peripheral to the present argument, but it
is no longer a speculative direction. OS-Copilot /
FRIDAY~\parencite{wu2024oscopilotgeneralistcomputeragents} already make room for user profile
...
Persona2Web~\parencite{kim2026persona2webbenchmarkingpersonalizedweb} makes the missing piece explicit by
...
\subsection{Main Gap: Procedural Memory for GUI}\label{61-main-gap-procedural-memory-for-gui}
...
from AppAgent~\parencite{zhang2023appagentmultimodalagentssmartphone} to
MobileGPT~\parencite{lee2024exploreselectderiverecall} to
MAGNET~\parencite{sun2026magnetadaptiveguiagents},
ActionEngine~\parencite{zhong2026actionenginereactiveprogrammaticgui}, and
IntentCUA~\parencite{lee2026intentcualearningintentlevelrepresentations} shows that GUI systems have
...
MobileGPT~\parencite{lee2024exploreselectderiverecall} demonstrates app-internal
procedural reuse, but its memory is explicitly per-app and per-device.
MAGNET~\parencite{sun2026magnetadaptiveguiagents} demonstrates workflow-level procedural
...
abstraction. ActionEngine~\parencite{zhong2026actionenginereactiveprogrammaticgui} proves that
...
experiential rules. IntentCUA~\parencite{lee2026intentcualearningintentlevelrepresentations} shows that
...
are considered. AWM~\parencite{wang2024agentworkflowmemory} and related self-evolving
...
\subsection{Main Gap: Failure-Driven Write-Back}\label{62-main-gap-failure-driven-write-back}
...
problem from merely having memory. AppAgent~\parencite{zhang2023appagentmultimodalagentssmartphone}
and MobileGPT~\parencite{lee2024exploreselectderiverecall} demonstrate memory
construction,
but neither provides autonomous failure-driven revision.
MobileAgent-v2~\parencite{wang2024mobileagentv2mobiledeviceoperation} is especially
...
persistent memory. MAGNET~\parencite{sun2026magnetadaptiveguiagents} goes further by
...
MobileAgentV3.5~\parencite{xu2026mobileagentv35multiplatformfundamentalgui} and related
...
closing the GUI gap. AWM~\parencite{wang2024agentworkflowmemory} and
ExpeL~\parencite{zhao2024expelllmagentsexperiential} show that success-failure contrasts can
yield reusable experience abstractions. SkillRL~\parencite{xia2026skillrlevolvingagentsrecursive}
shows that failures can be distilled into generalized corrective skills
rather than stored as raw cautionary traces.
A-MEM~\parencite{xu2025amemagenticmemoryllm} shows that memory can be revised rather
...
\subsection{Supporting Gap: Episodic Memory for GUI}\label{63-supporting-gap-episodic-memory-for-gui}
...
argument. Generative Agents~\parencite{park2023generativeagentsinteractivesimulacra} and
MemGPT~\parencite{packer2024memgptllmsoperatingsystems} already show why long-term storage,
selective retrieval, and memory management matter, while GUI-side
results such as
M2~\parencite{yan2026m2dualmemoryaugmentationlonghorizon} suggest that summarized history and insight
...
\subsection{Extension Gap: User-Centric Memory}\label{64-extension-gap-user-centric-memory}
...
path. OS-Copilot / FRIDAY~\parencite{wu2024oscopilotgeneralistcomputeragents} show that user
information can be acknowledged as part of an agent memory architecture,
and
Persona2Web~\parencite{kim2026persona2webbenchmarkingpersonalizedweb} demonstrates that
...
\subsection{Why These Gaps Matter More Than Other Blanks}\label{65-why-these-gaps-matter-more-than-other-blanks}
...
\section{Methodological Implications}\label{7-methodological-implications}
...
\subsection{How Memory Should Be Retrieved and Applied}\label{72-how-memory-should-be-retrieved-and-applied}
...
\subsection{Why Failure Must Trigger Memory Revision}\label{73-why-failure-must-trigger-memory-revision}
...
\subsection{Minimum Evaluation Package}\label{74-minimum-evaluation-package}
...
\section{Project Objectives and Proposed Methodology}\label{8-project-objectives-and-proposed-methodology}
...
\subsection{Initial Benchmark and Expected Outcome}\label{85-initial-benchmark-and-expected-outcome}
...
\subsection{What Would Count as Real Memory Gain}\label{91-what-would-count-as-real-memory-gain}
...
\subsection{Limits of the Current Survey}\label{92-limits-of-the-current-survey}
...
\subsection{ME/CE Result}\label{44-mece-result}

This taxonomy is intended as a diagnostic framework rather than a
one-label database schema, so its validity depends on a clear ME/CE
interpretation. At the level of \texttt{memory\ unit} or
\texttt{capability\ pattern}, the taxonomy passes the relevant test.
\texttt{What} and \texttt{When} are sufficiently distinct to classify
what kind of experience is being stored and at what stage it is being
consolidated. The earlier one-layer version of \texttt{How} does not
pass this test because it mixes matching strategies with application
mechanisms. Once \texttt{How} is split into
\texttt{Match\ Operator\ ×\ Application\ Carrier}, the categories become
internally coherent enough to support a stable analysis.
...
\subsection{Occupancy Summary}\label{45-occupancy-summary}
...
\section{Discussion}\label{8-discussion}

\subsection{What Would Count as Real Memory Gain}\label{81-what-would-count-as-real-memory-gain}
...
\subsection{Limits of the Current Survey}\label{82-limits-of-the-current-survey}
...
several peripheral or recently emerging systems remain only partially
integrated into the current literature synthesis. Their absence does not
overturn the core argument, but it does mean that the taxonomy should
still be read as a strong draft rather than a final census of the field.
...
\subsection{Outlook}\label{83-outlook}

The main practical value of this survey is that it converts a broad
topic area into a narrower research program. Instead of asking whether
GUI agents need memory in general, it argues that the most promising
next target is \texttt{experience-dependent\ procedural\ memory} linked
to \texttt{failure-driven\ write-back}. The result is not only a
literature summary, but a structured bridge toward subsequent
methodological development: a taxonomy that localizes the missing
capability, an evidence-backed gap analysis that prioritizes procedural
memory together with failure-driven write-back, and a methodological
agenda that already constrains what should be stored, how it should be
reused, and how it should be evaluated. The next step is to compress
this research agenda into a concrete method and experimental package,
not to reopen the problem framing at a broader and less testable level.
...
\section{Project Outline and Proposed Methodology}\label{10-project-outline-and-proposed-methodology}

\subsection{Refined Research Question}\label{101-refined-research-question}

Building on the survey findings, the project is centered on the
following refined question:

\begin{quote}
How can LLM-based GUI agents represent interaction-derived,
experience-dependent procedural knowledge as retrievable, revisable, and
cross-task reusable memory, and improve it through failure-aware
write-back from post-task experience, so that the gain extends beyond
repeated-task reuse to same-site or same-app cross-task reuse and
near-domain site-family or app-family transfer?
\end{quote}

This formulation imposes four methodological commitments. First, the
target memory must store interaction-derived procedural knowledge rather
than generic GUI priors. Second, the memory must survive beyond one
episode and contribute to later tasks. Third, failure must be treated as
a signal for memory maintenance rather than merely as an execution-time
exception. Fourth, empirical gains must extend beyond repeated-task
replay and remain visible under more demanding transfer settings.

\subsection{Proposed Memory Object}\label{102-proposed-memory-object}

The proposed method follows directly from the gap analysis in Sections 6
and 7. The intended memory object is a local
\texttt{experience-delta\ procedural\ rule}, or EDPM, meaning a compact
rule that captures what the agent learned from one local interaction and
when that lesson should transfer. This is different from storing a raw
trajectory, a frozen workflow cache, or a general semantic note. A
memory unit of this kind should record at least five fields: a trigger
condition, a recommended procedure, relevant constraints or
preconditions, a failure signal, and an explicit transfer scope. This
representation is meant to preserve the local rule extracted from
experience while keeping its boundary conditions visible enough to
support later revision.

The central rationale for this design choice is that current GUI memory
artifacts are usually either too concrete or too coarse. Raw
trajectories preserve detail but transfer poorly. Workflow-level
memories improve reuse but often conflate multiple local decisions into
one broad template. By contrast, the EDPM formulation aims to isolate
the smallest reusable policy fragment that remains actionable under
interface variation.

\subsection{Proposed System Design}\label{103-proposed-system-design}
...
incidental episode detail. The second is \textbf{rule abstraction},
which converts those segments into compact memory units with explicit
conditions of use. The third is \textbf{retrieval-application
separation}, which distinguishes how memory is matched from how it
influences the agent, allowing retrieval quality and application policy
to be evaluated independently. The fourth is \textbf{failure-aware
write-back}, which updates existing memory when later evidence shows
that a previously stored rule was incomplete, over-broad, or incorrect.
In plain terms, the system must decide what is worth storing, how to
compress it into a rule, how to use it later, and how to revise it when
it proves wrong.

This design is intentionally different from nearby alternatives. It does
not simply repackage app-bound knowledge documents, workflow memories,
or structural state graphs. It also does not treat success-only
accumulation as sufficient evidence of an evolving experience layer.
Finally, it does not equate training-time self-improvement with
deployment-time memory revision. The method is therefore positioned as a
memory-layer contribution rather than as a broader scaling or
post-training strategy.

\subsection{Evaluation Methodology}\label{104-evaluation-methodology}

The evaluation plan is designed to test whether the proposed memory unit
offers real procedural reuse rather than a stronger form of cache
lookup. The comparison conditions are: \texttt{C0} no memory,
\texttt{C1} raw retrieval, \texttt{C2} coarse workflow memory,
\texttt{C3} EDPM with success-only accumulation, \texttt{C4} EDPM with
failure-aware write-back, and \texttt{C5} EDPM combined with a stronger
policy constraint. These baselines isolate different alternative
explanations, including generic context gain, workflow-level reuse, and
the possibility that failure-aware revision adds no measurable value.

Evaluation will be staged across three transfer levels: \texttt{L1}
repeated-task reuse, \texttt{L2} same-site or same-app cross-task reuse,
and \texttt{L3} near-domain site-family or app-family transfer. The most
important hypotheses are that \texttt{C3} should outperform both
\texttt{C1} and \texttt{C2} on \texttt{L2}, and that \texttt{C4} should
outperform \texttt{C3} on failure-first episodes. A further guardrail is
that the method should preserve positive gain at \texttt{L2} without
producing negative overall transfer at \texttt{L3}. Under this design,
improvement limited to near-duplicate replay would not count as a
sufficient answer to the research question.

\subsection{Benchmark and Expected Outcome}\label{105-benchmark-and-expected-outcome}
...
are narrower than unrestricted cross-app generalization, but they are
methodologically suitable for determining whether the proposed memory
representation captures reusable procedural knowledge rather than
one-off workflow hints.

The expected outcome of the project is not merely a performance increase
on a single benchmark. The intended contribution is a methodologically
grounded demonstration that GUI agents can store interaction-derived
procedural rules as revisable memory units, retrieve them beyond literal
replay, and improve them when failures reveal incorrect scope or missing
constraints. If successful, the project should contribute both a
concrete memory design for GUI agents and an evaluation logic for
distinguishing genuine memory gain from prompt expansion, plan caching,
or success-case accumulation.
