...
step-level and case-level analysis. Rather than stopping at aggregate
score comparison, the study evaluates the paper's main Mind2Web claims
and then traces the method's effects through step-level decomposition, a
475-step paired-case analysis, target-site result tracing in the online
generalization setting, workflow text
...
claims rather than its broadest \textbf{performance} claims. We confirm
that LM-induced workflows are consistently more abstract than
rule-induced workflows at the text level, that prompt-level code and
text workflows behave similarly, and that induced workflow libraries are
compact and low-overlap. In contrast, in our first-run evidence, offline AWM is mixed rather than
uniformly superior across seven sites, online AWM does not become
stronger under larger distribution shift, and the claim that
natural-language observations are generally better than HTML is not
...
domain misdirection, template step-skipping, and workflow-first behavior
on mismatched sites. We identify eight boundary conditions not discussed
in the original paper and summarize a mechanism-consistent interpretive
...
The most defensible conclusion is that, on Mind2Web, AWM is best
understood as a \textbf{conditionally effective prompting mechanism}
rather than a uniformly reliable memory method. Its core idea survives
reproduction better than its strongest performance narrative.
...
and prompt-level case studies as complementary evidence modalities.

\section{The AWM Method}\label{the-awm-method}
...
\node[box] (taskt) {Current workflow file\\\(W_{<t}\)};
\node[box, right=of taskt] (infer) {Run inference on\\task \(t\)};
\node[tinybox, below=0.9cm of infer] (json) {Save result JSON\\for task \(t\)};
\node[box, right=of infer] (prefix) {Prefix results\\\(\{0,\dots,t\}\)};
\node[box, right=of prefix] (induce) {Re-induce workflows\\from all available\\prefix JSONs};
\node[box, below=0.9cm of induce] (update) {Overwrite workflow file\\with new summary\\\(W_{\le t}\)};
\node[box, below=0.9cm of prefix] (next) {Next task uses\\updated memory};

\draw[flow] (taskt) -- (infer);
\draw[flow] (infer) -- (prefix);
\draw[flow] (infer) -- (json);
\draw[flow] (json.east) -| (induce.west);
\draw[flow] (prefix) -- (induce);
\draw[flow] (induce) -- (update);
\draw[flow] (update.west) -- (next.east);
...
earlier tasks only, not from its own trajectory. Under
\texttt{induce\_steps = 1}, task \(2\) is therefore evaluated using a
workflow file re-induced from the completed result JSONs of tasks \(0\)
and \(1\). The update is overwrite-based over the full available prefix,
rather than an append-only ``old workflow + newest trajectory'' merge.
...
\textbf{Representation ablations} & Code/text difference is small; NL \textgreater{} HTML
observation & Code/text on kayak; NL/HTML on 3 websites \\
...
three sites (see the appendices on LM-vs.-rule text evidence,
concrete-value counting, and workflow counting conventions).
...
\textbf{Task:} ``Show the balance sheet and cash flow statement for
fiscal year 2021 of Six Flags.''
...
{\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2000}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2000}}@{}}
\caption{Site-level structural comparison.}\label{tab:site-level-structural-comparison}\\
...
the appendix on site-feature qualitative coding.
...
suggests. The appendix on Mind2Web compositionality provides the
...
\item
  On matched sites, workflows genuinely improve local step-level
  execution with zero negative interventions.
\end{itemize}
...
After reproduction, the paper's broadest claims can be rewritten more
carefully:

\begin{itemize}
\tightlist
\item
  AWM is not uniformly effective; it is \textbf{conditionally
  effective}.
\item
  Workflow usefulness is sparse rather than pervasive, changing behavior
  on only 6--18\% of steps.
\item
  Online memory is not broadly superior under shift; it is \textbf{more
  test-proximate but more brittle}.
\item
  Abstraction helps, but only under the right operational-divergence
  conditions.
\item
  AWM's gains come from specific mechanisms (strategy redirection, value
  guidance, termination prevention), not from generic ``high-level
...
These limitations should be understood as reporting discipline, not as
hidden caveats.
...
The method has a real mechanism. Reusable workflows can improve
step-level decisions, especially through TYPE-side value guidance and
matched CLICK-side grounding. LM-induced workflows are genuinely more
abstract than rule-induced ones, and workflow libraries are indeed
compact. These are substantive, reproducible aspects of the paper's
story.
...
\textbf{AWM is a plausible and interpretable prompting mechanism, but
not a uniformly reliable memory method. Its core idea survives
reproduction better than its strongest universality claims.}
