...
{\def\LTcaptype{} % do not increment counter
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{0.28\linewidth}>{\raggedleft\arraybackslash}p{0.46\linewidth}>{\raggedleft\arraybackslash}p{0.22\linewidth}@{}}
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{tripadvisor} & \texttt{qwen/qwen3.5-397b-a17b\ /\ online\_wf} &
4 & 18 & \texttt{+16pp} (\texttt{44.9\%} vs \texttt{29.2\%}) &
\texttt{78\%} & first-run underperformance with weaker baseline
candidate quality \\
\texttt{reddit} & \texttt{qwen/qwen3.5-397b-a17b\ /\ online\_wf} & 2 & 5
...
This case supports the step-level claim that, on a matched site,
workflow guidance can improve late-stage date-element grounding on a
matched interface.
...
This case supports the step-level claim that, on a matched site,
workflow guidance can redirect the model from the wrong action mode to
the correct one.
...
Task &
\texttt{Show\ the\ balance\ sheet\ and\ cash\ flow\ statement\ for\ the\ fiscal\ year\ 2021\ of\ Six\ Flags.} \\
\end{longtable}
}

\subsubsection{Claim supported}

This case supports the case-level claim that a semantically mismatched
workflow can pull the model into the wrong task family.
...
Task &
\texttt{Buy\ a\ single\ day\ pass\ to\ Six\ Flags,\ Magic\ Mountain.} \\
\end{longtable}
}

\subsubsection{Claim supported}

This case supports the case-level claim that a compressed
purchase-oriented template can induce step skipping.
