...
1. **TYPE 步骤收益最稳定**：即使在整体 not reproduced 的站点（budget），TYPE 的 Action F1 也有 +8.4% 提升。workflow 提供了"值模板"（例如城市名、搜索关键词），帮助模型选择正确的输入值。

2. **CLICK 步骤是胜负手**：reproduced 站点（kayak, newegg）在 CLICK 上有 +9~11% 的 Element Acc 提升；not reproduced 站点则反向退化 -6~12%。这说明 workflow 对 element grounding 的帮助**高度依赖站点匹配度**——workflow 中的元素描述如果匹配目标 HTML 结构就帮忙，不匹配就误导。

3. **后半段收益更大（reproduced 站点）**：kayak 后半段 +13% vs 前半段 +0%。这暗示 workflow 的价值不是"启动引导"而是"随着 trajectory 变长，帮助模型保持正确的操作模式"。

4. **not reproduced 站点后半段退化也更严重**：budget 后半段 -10.9%。workflow 的累积误导效应在长 trajectory 中被放大。
...
1. **reproduced 站点的 negative = 0**：workflow 在匹配站点上是"无害"的——要么帮忙，要么不影响，绝不误导。这是 AWM work 的关键前提。

2. **not reproduced 站点的 negative >> positive**：workflow 不匹配时变成有害噪声。budget 的 12 个 negative 中有 11 个是 CLICK（91%），说明 workflow 中的元素描述直接误导了 element grounding。

3. **ineffective 占比最大（45-65%）**：大部分步骤 workflow 既不帮忙也不妨碍——主要是 SKIP 步骤（ground truth 不在 candidate 中）和本身就困难的步骤。这意味着 **AWM 的有效窗口很窄**：只在一小部分步骤上产生影响，但在匹配站点上这个影响是净正向的。

4. **workflow 遵循率粗估**：在 positive + negative 步骤中（即 workflow 真正改变了模型行为的步骤），占总步骤的 6-18%。workflow 不是"每一步都在引导"，而是在关键步骤上偶尔介入。
...
- **判定**：混合退化——HTML 结构变化削弱了 baseline 的 candidate 质量（B），同时 workflow 从 kayak 归纳的操作逻辑不适配 tripadvisor 的导航结构（A）。
...
- **判定**：baseline 本身表现正常，退化主要来自 workflow 内容不适配。
...
**判定**：论文关于 LM vs Rule 抽象性差异的解释性主张**在文本层面完全成立**。这是 C1-C3 中得到最强支持的论文主张。
...
united 上 LM 确实优于 Rule（Rule 的具体值误导了模型），但 newegg 上 Rule 反而更好（Rule 的具体值恰好匹配了 test task 的操作模式，因为 newegg 的 cross-task 泛化场景中操作模式相对固定）。

这暗示：**抽象性的价值取决于 test task 与 train task 的操作模式差异程度**。差异大时，抽象有利；差异小时，具体案例反而更有用。论文没有展开讨论这个条件。
...
**判定**：text_wf 与 code_wf 的差异确实不大（Step SR 差 2.2pp），但**两者均低于 baseline**（51.2%）。论文声称"code vs text 差异不大"在方向上成立，但论文隐含的前提——两种表示都优于 baseline——在 kayak/qwen 上未成立。
...
**整体判定**：not reproduced after three-site first run。只有 newegg 呈现 `desc_only` 最优，但 kayak 与 united 都不支持论文主张，而且 newegg 的 `desc_html` 与 `desc_only` 基本持平，未形成论文要求的稳定收益结构。一个可能的解释是：united 的 HTML 元素命名本身就具有高可读性（如 `tab TRAVEL INFO`、`heading Check-in`），此时加入 NL 描述反而引入了冗余甚至噪声。

#### 5.2.4 论文未讨论的发现

1. **desc_html 并非"两全其美"**：在 united 上 desc_html 的 Step SR（51.7%）显著低于 html_only（60.6%）和 desc_only（55.9%），SR 从 33.3% 降到 16.7%。混合两种表示增加了 prompt 长度，可能导致注意力分散。
2. **表示层的最优选择是站点相关的**：newegg 适合 desc_only，united 适合 html_only。论文给出了一个跨站点的统一推荐，但实际上不存在一致最优的表示策略。
...
**判定**：supported（LM 和 online induction 产出紧凑的 workflow 库）。
...
**判定**：在当前近似统计口径下 supported，但 proxy 偏宽。prompt 注入率为 100%，而"实际被遵循"的比例从 §2 的配对分析看只有 6-18%。因此，这里支持的是“workflow 进入 prompt 并具备可用性”的宽口径 utility，而不是“workflow 被模型真实遵循”的强口径 utility。

**主张 3："function overlap 较低"**

LM workflow 的 function overlap 极低（0-17%），每个 workflow 覆盖不同的操作类型（enter_location, select_date, apply_filter 等），无明显冗余。
...
**判定**：在当前近似统计口径下支持 coverage 足够高，但这与论文中“coverage 不必很高”的陈述并不完全等价。runbook 里的 coverage 统计更接近“workflow 文本进入 prompt 并在样本中可见”的宽口径 proxy，而不是严格的功能覆盖率。因此，这一项应被理解为：当前 workflow 具备高 prompt-level coverage proxy，而不是已经证明“低 coverage 也无损价值”。
...
**核心发现**：workflow 数量本身不是决定因素。关键是**workflow 内容与 test task 操作模式的匹配度**。newegg 的 Rule workflow（19 个）反而比 LM（5 个）性能更好，因为 newegg 的 cross-task 操作模式相对固定，具体案例直接可用。
...
| **低 coverage 天花板** | 5-7 个抽象 workflow 无法覆盖大部分 test 步骤 | 6 | 结构性：45-65% 步骤无 workflow 可用 |

### 7.2 各类失败的占比统计

基于 paired_case 分析（gpt-4o/test_task 7 站点，offline_wf vs no_workflow，共 523 个配对步骤）：
...
1. **值模板效应（TYPE 步骤）**：workflow 为 TYPE 操作提供了输入值的格式和类型提示（如"搜索地点名"、"输入价格上限"），帮助模型选择正确的输入值。这个效应在所有站点上都是正向的，即使在整体 not reproduced 的站点上也有 +8% 的 Action F1 提升。具体案例：P-3（newegg）中 workflow 引导模型输出 `SELECT [Most Reviews]` 而非 `SELECT [5]`。

2. **元素定位效应（CLICK 步骤）**：workflow 中的元素描述（如 `[checkbox] {filter_option}`）帮助模型在候选元素中选对目标。但这个效应**高度依赖站点匹配度**——workflow 描述与实际 HTML 结构一致时帮忙（kayak +10.7%），不一致时反而误导（budget -12.1%）。

3. **策略重定向**（§9 新发现）：workflow 在关键决策点提供正确的操作模式。P-2 中 workflow 将模型从"浏览分类"策略重定向为"搜索优先"策略。P-1 中 workflow 阻止了模型的 premature termination。这类收益在聚合指标中不可见，但在 prompt-level 案例中清晰呈现。

4. **有效窗口窄但无害**：workflow 只在 6-18% 的步骤上真正改变模型行为。在匹配站点上，这些改变是净正向的（negative = 0）。AWM 不是"每一步都在引导"，而是"在关键步骤偶尔介入，且介入时不犯错"。

5. **后半段辅助效应**：在匹配站点上，workflow 的收益集中在 trajectory 的后半段（kayak +13% vs 前半段 +0%），暗示 workflow 帮助模型在长 trajectory 中保持正确的操作模式，而不是提供"第一步怎么做"的启动引导。

### 层次三结论：论文没有说清楚的部分

1. **workflow 在不匹配站点上是有害的**（§2, §3）：论文只报告了 online AWM 在 cross-site 上优于 baseline，但我们的复现显示 tripadvisor 和 reddit 上 online 均低于 baseline。workflow 从源站归纳的操作逻辑在目标站产生负迁移，negative 步骤远多于 positive（tripadvisor: 18 vs 4）。论文没有讨论 workflow 何时有害。

2. **抽象性优势是有条件的**（§4）：LM workflow 在文本层面确实更抽象（4/4 指标支持），但性能优势取决于 test task 与 train task 的操作模式差异程度。在操作模式固定的站点（newegg），具体的 Rule workflow 反而更好。论文只报告了 LM > Rule 的总体结论，没有讨论这个边界条件。

3. **workflow 的有效窗口很窄**（§2）：大部分步骤（45-65%）属于 ineffective，workflow 既不帮忙也不妨碍。AWM 的聚合分数提升掩盖了一个事实：workflow 真正起作用的步骤很少。论文用聚合分数呈现结果，没有报告 workflow 的实际影响面。

4. **SKIP 步骤的影响**（§7）：37% 的步骤因为 ground truth 元素不在 candidate 中被强制跳过并计为 0 分。这些步骤拉低了所有 condition 的绝对分数但不影响相对比较。然而，不同站点的 skip rate 差异很大（kayak 29% vs tripadvisor 45%），这意味着跨站点的绝对分数不可直接比较。论文没有报告 skip rate 的站点间差异。

5. **累积误导效应**（§1）：在 not reproduced 站点上，workflow 的误导在 trajectory 后半段被放大（budget 后半段 -10.9%）。这与匹配站点上的"后半段辅助效应"恰好相反，暗示 workflow 的正/负影响都会随 trajectory 长度累积。论文没有分析 workflow 影响的时序特征。

6. **表示层最优策略是站点相关的**（§5）：论文统一推荐"Desc only 优于 HTML"，但 3 站点中仅 newegg 支持，united 上 html_only 反而最好（因为 HTML 元素命名本身可读性高）。此外 desc_html 混合表示在 united 上导致 SR 从 33.3% 降至 16.7%，论文没有讨论混合表示可能带来的注意力分散问题。

7. **workflow 收益天花板很低**（§6）：5-7 个抽象 workflow 只在 6-18% 的步骤上真正改变了模型行为，45-65% 的步骤属于 ineffective。论文用聚合分数呈现 AWM 的提升，但没有讨论这种"稀疏影响"的结构特征。这意味着 AWM 的实际价值窗口远比聚合数字暗示的要窄。

8. **对齐率与性能呈反向关系**（§10）：workflow-target action 的表层对齐率在 not reproduced 站点反而最高（budget 98.6%, sixflags 94.2%），reproduced 站点反而较低（kayak 85.3%, newegg 75.0%）。这打破了"高覆盖=高性能"的直觉。原因是 budget/sixflags 的高对齐率反映了 action type 的表层关键词匹配，而非 task intent 的语义匹配。论文用 coverage 和 utility 等 proxy 指标评估 workflow 质量，但未讨论这类 proxy 的局限性。

9. **AWM 成功需要双重条件**（§10, §11）：workflow 可复用性（短小参数化 sub-routine，而非完整任务序列）+ baseline 提升空间（CLICK accuracy 65-72%，模型确实需要帮助）。缺少任一条件，AWM 要么无效（sixflags baseline 已高），要么有害（budget workflow 域外误导）。论文没有讨论这一成功前提。
...
**Task:** "Find hotels in Las Vegas, NV that offer free airport shuttle service."

| | baseline | workflow |
|---|---|---|
| step_success | 0 | **1** |
| 模型输出 | 自然语言总结："The cheapest small car rental deal… $381." | `CLICK [57318]`（Sort by Cheapest） |
...
**Task:** "Find a gaming desktop with an rtx4000 GPU, available in stock."
...
**Task:** "Remove the SSD on my cart"
...
**误导机制**：`select_park` workflow 第一步就是 `[button] Browse the Parks Below -> CLICK`，页面上恰好有这个按钮，模型优先执行 workflow 而非推理正确的 Investors 导航。
...
**误导机制**：`browse_ticket_options` workflow 只有 2 步（CLICK Tickets → CLICK 具体选项），跳过了中间的日历/日期选择步骤。模型将当前状态映射到模板的后续步骤，导致"跳步"。
...
**关键发现**：正向机制的核心是 workflow 在关键决策点提供了**正确的操作模式**（搜索而非浏览、标签而非索引、继续而非终止）。负向机制的核心是 workflow 提供了**不适用的操作模式**——且模型倾向于遵循 workflow 而非独立推理。这解释了 §2 中"reproduced 站点 negative=0"的现象：只要 workflow 内容匹配当前站点，它的"模式提示"就是无害的；一旦不匹配，模型对 workflow 的"顺从"就变成了误导源。
...
| Avg steps/WF | **2.5** | **3.5** | **5.5** | 3.6 |
| WF specificity | 参数化、模块化 | 参数化、模块化 | 高度特化（租车+求职+保险） | 泛而浅（选公园+浏览票） |
| Task diversity | 高（航班+酒店+租车+筛选） | 高（搜索+筛选+购物车+排序） | 异质（跨多个站点功能区） | 低（公园+门票变体） |

### 10.2 Workflow-Target 对齐率的反直觉发现

| 站点 | 对齐率 | C1 状态 | 解释 |
|------|------:|------|------|
| budget | **98.6%** | not reproduced | workflow 覆盖几乎所有 action type，但覆盖的是租车操作——test task 涉及保险、求职等域外功能 |
| sixflags | **94.2%** | not reproduced | workflow 覆盖了公园导航操作——但 test task 涉及财务报告 |
| kayak | **85.3%** | reproduced | workflow 覆盖主要搜索流程，少数二级 TYPE 操作未覆盖 |
| newegg | **75.0%** | reproduced | SELECT/排序操作覆盖最低，但恰好是收益最大的操作类型 |

**对齐率与复现状态呈反向关系**。这打破了"高对齐=高性能"的直觉假设。

**解释**：budget 和 sixflags 的高对齐率是"虚高"——workflow 模板覆盖了大量租车/公园操作的 CLICK 关键词，但 test task 中的保险/财务类任务使用了这些关键词的*不同语义实例*。对齐率度量了 action type 的表层匹配，而非 task intent 的语义匹配。

### 10.3 AWM 成功预测假说

**AWM 有效性 = f（Workflow 可复用性 × Baseline 提升空间）**

AWM 产生正向收益需要**两个条件同时满足**：

1. **Workflow 可复用性**：workflow 必须是短小、参数化的 sub-routine，能匹配多个 test task。
   - kayak/newegg 的 workflow（2-3 步/WF）是模块化的"操作字典"
   - budget 的 workflow（5.5 步/WF）包含完整的特定任务序列，不可复用
   - sixflags 的 workflow 虽短但过于泛化（"选公园→点按钮"），对模型已知的简单导航无增量价值

2. **Baseline CLICK 提升空间**：站点的 baseline CLICK accuracy 处于中等水平（65-72%），说明模型在元素识别上有困难。
   - sixflags baseline CLICK 73.5%——模型已经能正确导航简单 UI，workflow 只增加噪声
   - kayak/newegg baseline CLICK 67-71%——模型需要帮助，workflow 的元素描述提供了有效线索

**辅助因素**：
- **Task 异质性**：budget 的 test task 横跨站点多个功能区（租车、保险、求职），没有单一 workflow 集合能覆盖。kayak/newegg 的 task 虽多样但共享核心搜索模式。
- **CLICK 占比**：sixflags 非 SKIP 步骤中 94% 是 CLICK，workflow 对 CLICK 的影响高度依赖站点匹配度（§1），因此高 CLICK 占比放大了 mismatch 风险。
...
1. **归纳阶段**（offline/online）产生 workflow 库。LM induction 产出更抽象的 sub-routine（§4），rule induction 产出更具体的案例库。

2. **匹配阶段**（test time）：workflow 被注入 prompt。关键变量不是 workflow 数量或对齐率，而是 **task-workflow 语义匹配度**——workflow 描述的操作模式是否适用于当前 step。

3. **影响阶段**：在少量步骤（6-18%）上，workflow 真正改变了模型行为。
   - 匹配时：正向介入——策略重定向、值格式校正、防止过早终止
   - 不匹配时：负向干扰——域外误导、模板跳步、模型盲从 workflow 而非独立推理

4. **累积阶段**：正/负影响在 trajectory 中累积。后半段步骤受前序决策影响，导致匹配站点后半段 +13%、不匹配站点后半段 -10.9%。

5. **聚合结果**：最终 Step SR delta = Σ(positive) - Σ(negative)。reproduced 站点 negative=0 → 净正向；not reproduced 站点 negative >> positive → 净负向。

### 11.3 论文叙事 vs 实际机制

| 论文叙事 | 实际机制 |
|---------|---------|
| AWM 普适有效 | AWM 有条件有效：需要 workflow 可复用性 + baseline 提升空间 |
| online AWM 在大 distribution gap 下更优 | online AWM 在 cross-site 上产生负迁移（workflow 过拟合源站） |
| workflow 提供"高层操作指导" | workflow 在少数步骤上改变模型行为（6-18%），通过具体的策略重定向/值模板/终止控制起效 |
| LM induction 优于 rule induction | 抽象性优势依赖 task 差异程度（差异小时具体案例更好） |
| 高 utility rate | 真实遵循率 6-18%；对齐率与性能呈反向关系 |
...
§9-§11 的分析基于手动逐条对比 JSON 日志完成，产物直接写入本文档。
