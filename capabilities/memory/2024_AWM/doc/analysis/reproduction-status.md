...
- `C2`：整体方向偏向 `not reproduced`（仅 test_task/kayak 成功；两个 target-site 首轮结果均低于 baseline）
...
```text
C1-C5 已全部完成首轮复现。五条核心主张中，得到最强支持的是 C3 的解释性结论（LM workflow 更抽象），以及 C4 的 code/text 差异不大、C5 在当前近似统计口径下所呈现的 workflow 库紧凑与低冗余。得到最弱支持的是 C2 的 online-under-larger-gap 主张（当前 target-site 首轮结果方向相反）和 C4 的 NL vs HTML 主张（三站点首轮整体不支持）。C1 的核心性能主张稳定性不足。整体而言，AWM 的方法论设计有其合理性（LM 归纳确实更抽象），但性能收益不稳定且高度站点相关，论文呈现的“普适有效”图景在当前复现中未能成立。
```
...
3. C2 的两个 target-site 首轮结果均未复现论文中的 online-over-baseline 趋势；相关 `A3/B6` 材料应按结果追踪与 prompt-level case 读取，而不是独立机制证明
