...
- **Links**: [PDF](../source/SKILLRL.pdf) | [Code](https://github.com/aiming-lab/SkillRL)
...
- **My observed limitations**:
  SkillRL 已经把“经验蒸馏成技能 + 用 RL 继续演化”这条路线走通了，但它也清楚说明了现实边界：teacher 依赖强、环境仍偏文本/结构化、奖励信号是二值的，而且动态演化的边际收益小于高质量初始技能库本身。按当前主线，它是很重要的 blueprint，但不能直接当作 GUI 侧已解决。
...
- **Corresponding survey section/category**:
  这篇主要归入 `Self_Evolve` 中的 “Experience-driven Skill Discovery + RL Co-evolution” 子节，同时应在 `Agent_Memory` / `Cross_Topic` 中交叉引用。原因很直接：SKILLBANK 本质上就是 procedural memory，只是目前证据仍停留在文本 / 结构化环境。
...
- Gap signal 3: 实验覆盖 ALFWorld、WebShop 与 search-augmented QA 等文本/结构化环境（Section 4, p.4-5），并在实现中明确使用 "OpenAI o3 ... as the teacher model for skill distillation and SFT data generation" (Section 4, p.5) → skill 蒸馏对高能力 teacher 与非视觉环境的依赖仍很强，开放 GUI 场景的 teacher-free 路线尚未验证。

  对当前主线，最重要的不是“SkillRL 也还没做 GUI”这一点，而是两个结构性问题：初始 skill 质量与后续演化谁更关键，以及 teacher-heavy distillation 能否被更轻量的 runtime memory 替代。这两点都直接关系到 A-1 / A-4 的工程可行性。
...
> 这篇最可迁移的是失败轨迹四元组和两层 skill 结构。若迁移到 GUI，四元组完全可以作为 error-recovery skill 的骨架，而 general vs task-specific 也自然对应到 cross-app heuristic 与 app-family / app-specific procedure 两层。
...
### Connections to Other Papers in Knowledge Base
  ...
  它与 [2025_EvolveR.md](/Users/mac/studyspace/Knowledge-Markdown/Self_Evolve/papers/notes/2025_EvolveR.md) 的差异是 skill abstraction vs raw / principle-centric memory，与 [2026_MemRL.md](/Users/mac/studyspace/Knowledge-Markdown/Self_Evolve/papers/notes/2026_MemRL.md) 的差异则是 skill co-evolution vs memory-retrieval policy RL。对 `Cross_Topic` 而言，它是 A-1 / A-4 最强的一批正面先行工作之一，但证据域仍主要停留在非 GUI 环境。
