...
- **Links**: [PDF](../source/GUI-GENESIS.pdf) | Code: not listed in PDF | Project: not listed in PDF
...
- **My observed limitations**: 
  GUI-Genesis 的贡献在于“造训练环境”，不是“造记忆机制”。它依赖 code-level oracle 和 mini-app setting；一旦迁移到更复杂的 desktop / open web 场景，reward 可验证性与环境 fidelity 都会显著变难。因此它更适合被理解为训练基础设施，而不是对 A-1 / A-4 的直接回答。
...
- **Corresponding survey section/category**:
  这篇应放在 GUI Agent survey 的 **Training Environments / Post-Training / Self-Evolution** 小节。它的定位应明确为基础设施论文，用来支撑“为什么 training-side evolution 有机会继续扩大”，而不是 runtime memory 方案。
...
- Gap signal 3: "the model can synthesize environments that it cannot yet solve" (Section 6.3, p.7) → 环境生成能力本身可以变成自动 curriculum 的来源，这是 self-evolving GUI agents 的强研究信号。
  ...
  如果知识库要完整覆盖 self-evolution，这篇很关键，因为它把“数据 / 环境生产”从人工瓶颈推向了 agent-generated infrastructure；但在当前 main-line 下，它只提供训练侧支撑，不改变 deployment-time memory 缺口的判断。

### Reusable Elements
- **Methodology**: trace-driven environment synthesis、code-native reward injection、sim-to-real controlled comparison 都非常可复用。
  最可迁移的是 code-native reward / deterministic checker 思路。即使不完整复现环境合成，也可以把部分 GUI 子目标包成可验证子任务，用来辅助经验筛选或失败归因。
...
### Connections to Other Papers in Knowledge Base
  它与 [2026_ACuRL.md](/Users/mac/studyspace/Knowledge-Markdown/GUI_Agent/papers/notes/2026_ACuRL.md) 一起构成 training-side 路线，与 [2025_MobileAgentV3.md](/Users/mac/studyspace/Knowledge-Markdown/GUI_Agent/papers/notes/2025_MobileAgentV3.md) 共同说明 GUI 自演化已经从“造数据”走向“用数据训练”。再和 [2026_ActionEngine.md](/Users/mac/studyspace/Knowledge-Markdown/GUI_Agent/papers/notes/2026_ActionEngine.md) 对照时，training infrastructure 和 runtime planning / memory 的差异就很清楚。
